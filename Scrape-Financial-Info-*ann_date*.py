...
def downLoadData(pro):
    ...
    #Breakpoint Recovery
    try:
      sql = '''SELECT success_codelist FROM Progress'''
      cursor.execute(sql)
    except:
      sql = "CREATE TABLE IF NOT EXISTS Progress(success_codelist)"
      cursor.execute(sql)
    success_codelist = cursor.fetchall()

    #Financial Table
    financial = pd.DataFrame()
    codelist = pro.stock_basic()
    for i in tqdm(range(len(codelist["ts_code"]))):
      if (codelist["ts_code"][i],) in success_codelist: continue
      else:
